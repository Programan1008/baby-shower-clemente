...
function renderDone(record) {
  ...
  $("finalMessage").textContent = record.asistencia
    ? "Tu asistencia ha sido confirmada y registrada. Nos vemos para celebrar la llegada de Clemente."
...
function prepareEdit(record) {
  ...
  $("detailsCopy").textContent =
    "Puedes actualizar el nombre registrado o volver para cambiar Sí/No.";
  showStep("details");
}
...
function resetDemo() {
  ...
  showStep("attendance");
  showToast(
    "Se limpió este dispositivo. El registro remoto de prueba sigue en Firestore."
  );
}
...
$("rsvpForm").addEventListener("submit", async event => {
  ...
  try {
    ...
    showToast("No pudimos guardar la respuesta. Revisa tu conexión e inténtalo otra vez.");
  } finally {
    state.saving = false;
